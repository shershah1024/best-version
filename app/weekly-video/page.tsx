...
export default function WeeklyVideoPage() {
  ...
  const fetchWeeklyData = async (week: WeekOption) => {
    try {
      ...
      // 1. Check for existing video in future_videos table
      ...
      if (videoError && videoError.code !== 'PGRST116') { // PGRST116 is "not found" error
        throw new Error('Error fetching video data');
      }

      // If video exists, set it and we're done
      if (video?.video_url) {
      ...
      // 3. If no video exists, generate one
      if (!video?.video_url) {
        ...
        if (!videoResponse.ok) {
          throw new Error('Failed to process weekly video');
        }

        const videoData = await videoResponse.json();

        // Save to future_videos table
        ...
        if (insertError) {
          console.error('Failed to save video to database:', insertError);
          // Still set the video URL even if saving fails
        }
